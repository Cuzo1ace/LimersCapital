...
export default function PriceTicker() {
    ...
    return cryptoItems.map(t => {
      ...
      return (
        <span key={`${key}-${t.symbol}`} className="inline-flex items-center gap-1.5 px-4">
          ...
          {chg != null && (
            <span className="text-[.62rem] font-mono font-bold" style={{ color: up ? '#00ffa3' : '#FF4A6B' }}>
  ...
  function renderTTSE(key) {
    return ttseItems.map(s => {
      const up = s.chg > 0;
      const flat = s.chg === 0;
      const color = flat ? '#5B7A9A' : up ? '#00ffa3' : '#FF4A6B';
      return (
  ...
  return (
    <div className="border-b select-none" style={{ borderColor: 'rgba(0,255,163,0.12)' }}>
      ...
      {/* ── Divider ── */}
      <div style={{ height: '1px', background: 'rgba(0,255,163,0.08)' }} />
