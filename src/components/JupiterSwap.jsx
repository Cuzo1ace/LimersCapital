...
export default function JupiterSwap() {
  ...
  return (
    <div className="rounded-xl border border-sea/25 p-5" style={{ background: 'linear-gradient(135deg, rgba(56,189,248,.05) 0%, var(--color-card) 100%)' }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-[.65rem] text-muted bg-transparent border border-border rounded-lg px-2 py-1 cursor-pointer hover:text-txt hover:border-white/20 transition-all"
          >
      ...
      {/* Slippage settings */}
      {showSlippage && (
        <div className="mb-4 p-3 rounded-lg border border-border bg-black/20">
          <div className="text-[.6rem] text-muted uppercase tracking-widest mb-2">Slippage Tolerance</div>
          <div className="flex gap-1.5">
            {[10, 50, 100, 300].map(bps => (
              <button key={bps}
                onClick={() => setSlippage(bps)}
                className={`px-3 py-1.5 rounded-lg text-[.7rem] font-mono cursor-pointer border transition-all
                  ${slippage === bps ? 'bg-sea/15 border-sea/40 text-sea' : 'bg-transparent border-border text-muted hover:text-txt'}`}>
                {bps / 100}%
              </button>
            ))}
          </div>
      ...
      {/* Input token */}
      <div className="rounded-xl border border-border p-4 mb-2" style={{ background: 'rgba(0,0,0,.2)' }}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[.6rem] text-muted uppercase tracking-widest">You Pay</span>
        </div>
        <div className="flex items-center gap-3">
          <input
            ...
            placeholder="0.00"
            className="flex-1 bg-transparent border-none text-[1.4rem] font-headline font-bold text-txt outline-none w-0 min-w-0"
          />
          <select
            ...
            className="bg-black/40 border border-border text-txt rounded-xl px-3 py-2 font-mono text-[.8rem] outline-none cursor-pointer min-w-[100px]"
          >
      ...
      <div className="flex justify-center -my-1 relative z-10">
        <button onClick={handleFlip}
          className="w-9 h-9 rounded-full bg-sea/15 border border-sea/30 text-sea flex items-center justify-center cursor-pointer hover:bg-sea/25 transition-all text-base">
          ⇅
        </button>
      </div>

      {/* Output token */}
      <div className="rounded-xl border border-border p-4 mt-2 mb-4" style={{ background: 'rgba(0,0,0,.2)' }}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[.6rem] text-muted uppercase tracking-widest">You Receive</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex-1 text-[1.4rem] font-headline font-bold text-txt min-w-0">
            {quoteLoading ? (
              <span className="text-muted text-[1rem]">Fetching quote...</span>
            ) : outputAmount != null ? (
              fmtNum(outputAmount)
            ) : (
      ...
      {/* Quote details */}
      {quoteData && (
        <div className="rounded-lg border border-border p-3 mb-4 text-[.72rem]" style={{ background: 'rgba(0,0,0,.15)' }}>
          <div className="flex justify-between py-1 border-b border-white/5">
            <span className="text-muted">Rate</span>
            <span className="text-txt font-bold">
              1 {inputToken.symbol} ≈ {fmtNum(outputAmount / parseFloat(inputAmount))} {outputToken.symbol}
            </span>
          </div>
          <div className="flex justify-between py-1 border-b border-white/5">
            <span className="text-muted">Price Impact</span>
            <span className={priceImpact > 1 ? 'text-down font-bold' : priceImpact > 0.3 ? 'text-sun' : 'text-up'}>
              {priceImpact != null ? `${priceImpact.toFixed(4)}%` : '—'}
            </span>
          </div>
          <div className="flex justify-between py-1 border-b border-white/5">
            <span className="text-muted">Route</span>
            <span className="text-sea text-[.65rem]">{routePlan.length ? routePlan.join(' → ') : 'Direct'}</span>
          </div>
          <div className="flex justify-between py-1">
            <span className="text-muted">Slippage</span>
            <span className="text-txt">{slippage / 100}%</span>
          </div>
        </div>
      )}
      ...
      {/* Error */}
      {quoteError && (
        <div className="rounded-lg border border-down/30 bg-down/10 p-3 mb-4 text-[.74rem] text-down">
          {quoteError}
        </div>
      )}
      ...
      {swapStatus === 'success' && txSignature && (
        <div className="rounded-lg border border-up/30 bg-up/10 p-3 mb-4 text-[.74rem]">
          ...
          <a href={`https://solscan.io/tx/${txSignature}`} target="_blank" rel="noopener noreferrer"
            className="text-sea text-[.68rem] hover:underline break-all">
            View on Solscan →
          </a>
        </div>
      )}
      ...
      {/* Action buttons */}
      {!walletConnected ? (
        <div className="rounded-xl border border-border p-5 text-center" style={{ background: 'rgba(0,0,0,.2)' }}>
          <div className="text-2xl mb-2">🔗</div>
          <div className="text-[.82rem] font-bold text-txt mb-1">Connect Wallet to Swap</div>
          <div className="text-[.7rem] text-muted">Connect your Solana wallet to execute real on-chain swaps via Jupiter.</div>
        </div>
      ) : !quoteData ? (
        <button
          onClick={fetchQuote}
          disabled={!inputAmount || parseFloat(inputAmount) <= 0 || quoteLoading}
          className={`w-full py-3.5 rounded-xl text-[.88rem] font-headline font-bold border-none cursor-pointer transition-all
            ${!inputAmount || parseFloat(inputAmount) <= 0
              ? 'bg-border text-muted cursor-not-allowed'
              : 'bg-sea text-night hover:opacity-90'}`}
        >
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-3 rounded-xl text-[.82rem] font-headline font-bold border border-border bg-transparent text-muted cursor-pointer hover:text-txt transition-all"
          >
            ← New Quote
          </button>
          <button
            ...
            className="flex-[2] py-3 rounded-xl text-[.88rem] font-headline font-bold border-none cursor-pointer transition-all bg-up text-night hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {swapStatus === 'signing' ? '✍️ Signing...' :
