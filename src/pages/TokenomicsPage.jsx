...
export default function TokenomicsPage() {
  ...
  return (
    <div>
      {/* Hero */}
      <div className="rounded-2xl p-9 mb-7 border border-sea/20 relative overflow-hidden"
        ...
        <h1 className="font-serif text-[2.6rem] font-black leading-[1.05] text-txt mb-3">
        ...
        <div className="flex gap-4 mt-5 flex-wrap">
          ...
          <Stat label="Community Share" value="50%" color="#1DCC8A" />
          <Stat label="Current Phase" value="LP Accumulation" color="#FFCA3A" />
        </div>
      </div>

      {/* 50/50 Distribution */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Token Distribution — 50/50 Split</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-7">
        {[DISTRIBUTION.community, DISTRIBUTION.platform].map(side => (
          <div key={side.label} className="rounded-2xl border border-border p-6" style={{ background: 'var(--color-card)' }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-3 h-3 rounded-full" style={{ background: side.color }} />
              <h3 className="font-sans font-bold text-[1rem]" style={{ color: side.color }}>{side.label} — {side.pct}%</h3>
            </div>
            {/* Bar */}
      ...
      {/* Revenue Streams */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Revenue Streams</h2>
      ...
      {/* Staking Tiers */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Staking Tiers</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-7">
        {STAKING_TIERS.map(t => (
          <div key={t.name} className="rounded-2xl border border-border p-5 text-center" style={{ background: 'var(--color-card)' }}>
            <div className="text-3xl mb-2">{t.icon}</div>
            <div className="font-sans font-bold text-[1rem] mb-1" style={{ color: t.color }}>{t.name}</div>
      ...
      {/* Governance Roadmap */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Governance Roadmap</h2>
          ...
          <div key={g.phase} className={`rounded-xl border p-5 flex items-start gap-4
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-sans font-bold text-[.88rem] text-txt">{g.title}</span>
      ...
      {/* Value Props */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Why Hold {TOKEN.symbol}?</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3.5 mb-7">
        {VALUE_PROPS.map(v => (
          <div key={v.title} className="rounded-xl border border-border p-5" style={{ background: 'var(--color-card)' }}>
            ...
            <div className="font-sans font-bold text-[.84rem] text-txt mb-1">{v.title}</div>
      ...
      {/* CTA */}
      <div className="rounded-2xl border border-sea/30 p-8 text-center" style={{ background: 'linear-gradient(135deg, rgba(0,200,180,.06), rgba(45,155,86,.03))' }}>
        <div className="text-3xl mb-3">🍋</div>
        <h3 className="font-sans font-bold text-[1.2rem] text-txt mb-2">Start Earning Limer Points Today</h3>
...
function Stat({ label, value, color }) {
  return (
    <div className="bg-black/20 border border-border rounded-xl px-4 py-2.5">
      ...
      <div className="font-sans font-bold text-[1.1rem]" style={{ color: color || 'var(--color-txt)' }}>{value}</div>
    </div>
  );
}
