...
export default function ListingPage() {
  ...
  return (
    <div>
      {/* Hero */}
      <div className="rounded-2xl p-9 mb-7 border border-[rgba(200,16,46,.22)] text-center relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(200,16,46,.06), rgba(0,200,180,.04))' }}>
      ...
      {/* Benefits */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Why List on Limer's Capital?</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3.5 mb-7">
        {LISTING_BENEFITS.map(b => (
          <div key={b.title} className="rounded-xl border border-border p-5" style={{ background: 'var(--color-card)' }}>
            ...
            <div className="font-sans font-bold text-[.84rem] text-txt mb-1">{b.title}</div>
      ...
      {/* Listing Tiers */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Listing Tiers</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-7">
        {LISTING_TIERS.map(t => (
          <div key={t.name} className={`rounded-2xl border p-6 flex flex-col relative
            ...
            <h3 className="font-sans font-bold text-[1.1rem] mb-1" style={{ color: t.color }}>{t.name}</h3>
            <div className="flex items-baseline gap-1 mb-1">
              <span className="font-serif text-[1.8rem] font-black text-txt">{t.price}</span>
            ...
            <button onClick={() => { setForm({ ...form, tier: t.name }); document.getElementById('listing-form')?.scrollIntoView({ behavior: 'smooth' }); }}
              className="w-full px-4 py-2.5 rounded-xl text-[.82rem] font-sans font-bold cursor-pointer border transition-all"
              style={t.popular
      ...
      {/* TTSE Showcase */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Already Showcased on Our Platform</h2>
      <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-2 mb-7">
        {TTSE_FALLBACK.slice(0, 16).map(s => (
          <div key={s.sym} className="rounded-xl border border-border p-3 text-center" style={{ background: 'var(--color-card)' }}>
            <div className="font-sans font-bold text-[.82rem] text-[#FF4D6D]">{s.sym}</div>
      ...
      {/* Application Form */}
      <div id="listing-form">
        <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Apply to List</h2>
        {submitted ? (
          <div className="rounded-2xl border border-up/30 p-8 text-center bg-up/5">
            <div className="text-3xl mb-3">✅</div>
            <h3 className="font-sans font-bold text-[1.1rem] text-up mb-2">Application Submitted!</h3>
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl border border-border p-6" style={{ background: 'var(--color-card)' }}>
      ...
      {/* FAQ */}
      <h2 className="font-sans text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4 mt-7">Frequently Asked Questions</h2>
