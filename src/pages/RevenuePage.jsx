...
export default function RevenuePage() {
  ...
  return (
    <div>
      {/* Hero */}
      <div className="rounded-xl p-9 mb-7 border border-sea/20"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-7">
        ...
        <StatCard label="Community Pool (50%)" value={fmtUSD(simulatedRevenue.communityPool)} color="#00ffa3" />
        <StatCard label="Platform Pool (50%)" value={fmtUSD(simulatedRevenue.platformPool)} color="#FF5C4D" />
        <StatCard label="Your Share" value={fmtUSD(userRevenue)} sub={`${(userShare * 100).toFixed(4)}% of community pool`} color="#00ffa3" />
      </div>

      {/* 50/50 Visualization */}
      <h2 className="font-headline text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Revenue Flow</h2>
      ...
      {/* Revenue by Source */}
      <h2 className="font-headline text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Revenue by Source</h2>
      <div className="rounded-xl border border-border p-6 mb-7" style={{ background: 'var(--color-card)' }}>
      ...
      {/* Growth Simulator */}
      <h2 className="font-headline text-[.92rem] font-bold uppercase tracking-widest text-txt mb-4">Growth Simulator</h2>
      <div className="rounded-xl border border-border p-6 mb-7" style={{ background: 'var(--color-card)' }}>
        <div className="mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <MiniStat label="Community Annual" value={fmtUSD(communityAnnual)} color="#00ffa3" />
        </div>
      ...
      <div className="rounded-xl border border-sea/30 p-8 text-center"
        ...
        <h3 className="font-body font-bold text-[1.1rem] text-txt mb-2">Grow Your Share</h3>
...
function StatCard({ label, value, sub, color }) {
  return (
    <div className="rounded-[14px] p-5 border border-border" style={{ background: 'var(--color-card)' }}>
      ...
      <div className="font-headline text-[1.5rem] font-black" style={{ color }}>{value}</div>
...
function MiniStat({ label, value, color }) {
  return (
    <div className="bg-black/20 rounded-xl p-3 text-center">
      ...
      <div className="font-body font-bold text-[1rem]" style={{ color: color || 'var(--color-txt)' }}>{value}</div>
    </div>
  );
}
